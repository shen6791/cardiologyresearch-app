  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-sm text-[#8a97b8] mt-1">Manage every section of the public site from here.</p>
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((c) => (
          <Link key={c.href} href={c.href} className="glass rounded-2xl p-6 hover:border-[#4fe3c1]/50 transition">
            <div className="text-3xl font-display font-semibold text-[#4fe3c1]">{c.count}</div>
            <div className="text-sm text-[#8a97b8] mt-1">{c.label}</div>
          </Link>
        ))}
      </div>
    </div>
