        ...
        {items.map((f, i) => (
          <Reveal key={f.id} delay={i * 60}>
            <div className="card-lift border border-[#e6e3db] rounded-2xl p-7 flex items-center gap-6 flex-wrap">
              <div className="w-14 h-14 rounded-2xl bg-[#4a5d23] flex items-center justify-center flex-none">
              ...
              {f.pdf_url && <a href={f.pdf_url} target="_blank" rel="noopener" className="btn-lift px-5 py-2.5 rounded-md border border-[#d8d4c8] text-sm font-medium hover:border-[#4a5d23]">Learn More</a>}
            </div>
          </Reveal>
        ))}
