      ...
      <div className="max-w-5xl mx-auto px-6 pb-24 flex flex-col gap-4">
        {items.map((f, i) => (
          <Reveal key={f.id} delay={i * 60}>
            <div className="card-lift border border-[#e6e3db] rounded-lg p-7 flex items-center gap-6 flex-wrap">
              <div className="w-14 h-14 rounded-lg bg-[#0f5d52] flex items-center justify-center flex-none">
              ...
              {f.pdf_url && <a href={f.pdf_url} target="_blank" rel="noopener" className="btn-lift px-5 py-2.5 rounded-md border border-[#d8d4c8] text-sm font-medium hover:border-[#0f5d52]">Learn More</a>}
            </div>
          </Reveal>
        ))}
