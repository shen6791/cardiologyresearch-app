          ...
          <Reveal key={p.id} delay={Math.min(i * 60, 240)}>
            <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-[var(--bg-soft)] transition-colors">
              <div>
                ...
                {p.presented_at && <div className="text-sm text-[var(--muted)] mt-1">Presented at: {p.presented_at}</div>}
              </div>
              <div className="flex gap-4 flex-wrap flex-none">
                {p.presentation_links?.map((l) => (
                  <a key={l.id} href={l.url} target="_blank" rel="noopener" className="text-sm font-medium text-[var(--accent)] hover:underline">{l.label} →</a>
                ))}
              </div>
            </div>
          </Reveal>
        ))}
        {items.length === 0 && <div className="p-6 text-sm text-[var(--muted)]">No presentations listed yet.</div>}
      </div>
    </div>
  );
}
